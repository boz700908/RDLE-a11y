...
- [4. 基础操作](#4-基础操作)
  ...
  - [4.4 编辑事件](#44-编辑事件)
    - [4.4.1 移动事件](#441-移动事件)
    - [4.4.2 编辑事件属性](#442-编辑事件属性)
    - [4.4.3 高级编辑](#443-高级编辑)
    - [4.4.4 快捷操作](#444-快捷操作)
  - [4.5 网格系统](#45-网格系统)
  - [4.6 编辑光标](#46-编辑光标)
- [5. 高级功能](#5-高级功能)
  - [5.1 虚拟选区和事件链](#51-虚拟选区和事件链)
    - [5.1.1 虚拟选区](#511-虚拟选区)
    - [5.1.2 事件链](#512-事件链)
  - [5.2 条件系统](#52-条件系统)
  - [5.3 事件查找](#53-事件查找)
  - [5.4 标签模式](#54-标签模式)
  - [5.5 编辑关卡元数据](#55-编辑关卡元数据)
- [6. 外部编辑器（RDEventEditorHelper）](#6-外部编辑器rdeventeditorhelper)
  - [6.1 这是什么？](#61-这是什么)
  - [6.2 怎么用？](#62-怎么用)
- [7. 已知问题与局限](#7-已知问题与局限)
- [8. 常见问题](#8-常见问题)
  ...
  - [如何有效反馈问题？](#如何有效反馈问题)
  - [为什么 mod 没有加载？](#为什么-mod-没有加载)
  - [为什么 helper 打不开？](#为什么-helper-打不开)
...
- [9. 附录](#9-附录)
  - [9.1 原生编辑器常用快捷键一览表](#91-原生编辑器常用快捷键一览表)
  - [9.2 mod 快捷键一览表](#92-mod-快捷键一览表)
    - [菜单操作](#菜单操作)
    - [基础操作](#基础操作)
    - [编辑操作](#编辑操作)
    - [编辑光标](#编辑光标)
...
- 无需鼠标（全程支持键盘导航）
- 完整的语音反馈（与游戏原生共用相同的 narration 模块）
...
2. 虽然 mod 已经相对较为稳定，但依然建议你养成**随手保存**的习惯。我曾经遇到过莫名崩溃的情况（详情请看[这里](#为什么问我是否保存的对话框弹出来之后直接卡死了)），但还好我有这个好习惯。哎，都是被逼的。

## 2. 安装

### 2.1 系统要求

- windows 系统（别问我具体是什么版本，只要游戏能跑的起来 mod 大概也能跑得起来）
- rhythm doctor 游戏本体（1.0+，不支持独立版编辑器）
- 你的屏幕阅读器（这个不必多说）
...
解压开之后，你除了看到这个手册之外，应该还能看到**main**文件夹。点进去，全选复制到你的 rhythm doctor.exe 所在目录粘贴就搞定了。下次运行游戏的时候，mod 就可以顺利运行了。
...
在主菜单中找到**关卡编辑器，此功能还没有兼容朗读。**，enter 进入。
...
- z，事件前移 1 网格单位（加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节）
- x，事件后移 1 网格单位（加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节）
- c，将事件吸附到最近的网格
...
- r/t，调节属性的值（如果属性有多个可选项，则是在选项之间切换；如果是开关，则在启用/未启用之间切换；如果是整数，则调节步长为 1 且不支持修饰键；如果是浮点数，则默认步长为 0.1，加 shift 为 0.01，加 alt 为 0.001，加 shift 和 alt 为 0.0001）
...
- 对于循环大于0的单发节拍，按下后自动将其拆成多个独立的单发拍。
- 对于单个独立的单发节拍，按下后在编辑光标处创建一个相同参数的单发拍，并将击拍点与选中的单发拍同步。
- 对于普通节拍，按下后弹出一个网格大小输入框，确认后自动将拍长调整为对应的网格大小。
- 对于修饰拍子，按下后弹出一个菜单，可一件设置常用的x拍模式。
- 更多事件的快捷操作将陆续添加。
...
- ,（逗号），编辑光标前移 1 网格单位（加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节）
- .（句号），编辑光标后移 1 网格单位（加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节）
...
- -（减号）/=（等号），浏览虚拟选区中的事件（加 shift 为快速跳转到手项/尾项）
- ctrl+shift+x，剪切虚拟选区中的所有事件
- ctrl+shift+c，复制虚拟选区中的  所有事件
- ctrl+alt+空格，打开虚拟选区扩展菜单，这里是一些与虚拟选区相关但不那么常用的操作列表，上下光标浏览，回车执行。
...
- 浏览事件时，不再朗读事件位置，而是朗读事件的标签。
- 按ctrl+enter时，将打开一个仅支持标签编辑的对话框。
...
你可以在[9.3 联系方式](#93-联系方式)中找到联系我的渠道。
...
| shift+0 | 打开/关闭标签模式 | 详见 [5.4 标签模式](#54-标签模式) |
| esc | 退出全屏/取消选择 | 无 |
...
| alt+g | 打开网格设置菜单 | 详见[4.5 网格系统](#45-网格系统)。 |
| ctrl+f | 打开查找对话框 | 详见 [5.3 事件查找](#53-事件查找) |
| f3/shift+f3 | 以编辑光标为基准，向后/向前查找符合查找规则的事件 | 无 |

#### 编辑操作

| 快捷键 | 作用 | 备注 |
| --- | --- | --- |
| ctrl+enter | 打开 helper 编辑事件 | 无 |
| ctrl+shift+enter | 执行对应事件的快捷操作 | 无 |
| shift+enter | 打开 helper 编辑轨道 | 无 |
| 大键盘 0 | 打开 helper 编辑元数据 | 无 |
| insert 或 f2 | 在编辑光标处插入事件 | 无 |
| ctrl+insert 或 ctrl+f2 | 添加轨道/精灵到所在房间（仅轨道/精灵标签页有效） | 无 |
| z/x | 所选事件前移/后移 1 个网格单位（或 1 小节） | 加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节。详见[4.4.1 移动事件](#441-移动事件)。 |
| c | 事件吸附到最近的网格 | 无 |
| e/shift+e | 切换当前要调节的目标属性 | 详见[4.4.2 编辑事件属性](#442-编辑事件属性)。 |
| r/t | 调节属性的值 | 支持修饰键，详见[4.4.2 编辑事件属性](#442-编辑事件属性)。 |
| alt+c | 打开所选事件的条件菜单 | 详见[5.2 条件系统](#52-条件系统)。 |
...
如果这个 mod 有帮助到你，并且你希望支持我的开发工作，除了向更多人推荐该 mod，你也可以考虑给我一些资金上的赞助。请注意，这里的赞助是针对我**个人**的，与**7th beat games**完全无关。
