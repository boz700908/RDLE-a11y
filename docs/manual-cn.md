...
mod 提供了一组简单的快捷键来调整事件的节拍位置。这一组快捷键除了按键不同，调整的对象不同之外，和编辑光标有很多的共同点。如下：

- z，事件前移 1 网格单位（同样支持修饰键）
- x，事件后移 1 网格单位（同样支持修饰键）
- c，将事件吸附到最近的网格
...
因此，如果你需要实现更精确的调节，你可以按 ctrl+enter 打开外部编辑器进行编辑。有关外部编辑器的更多信息，请参见下一章的内容。
...
| shift+0 | 打开/关闭标签模式 | 详见 [4.10 标签模式](#410-标签模式) |
| esc | 退出全屏/取消选择 | 无 |
...
| ctrl+f | 打开查找对话框 | 详见 [4.9 事件查找](#49-事件查找) |
...
| z/x | 所选事件前移/后移 1 个网格单位（或 1 小节） | 加 shift 为 0.1 拍，加 alt 为 0.01 拍，加 shift 和 alt 为 1 小节。详见[4.7.1 移动事件](#471-移动事件)。 |
| c | 事件吸附到最近的网格 | 无 |
